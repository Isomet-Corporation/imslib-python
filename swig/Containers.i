...
template <typename CTYPE>
  class ListBase {
  %typemap(csinterfaces) ListBase "System.IDisposable, System.Collections.IEnumerable\n    , global::System.Collections.Generic.IList<$typemap(cstype, CTYPE)>\n";

  %typemap(cscode) ListBase %{
  ...
  %extend {
	  ...
	  ListBase<CTYPE>::iterator it = $self->begin();
      ...
      void AddRange(const ListBase<CTYPE>& values) {
        $self->insert($self->end(), values.begin(), values.end());
	}
      // Takes a deep copy of the elements unlike ArrayList.GetRange
      ListBase<CTYPE> *GetRange(int index, int count) throw (std::out_of_range, std::invalid_argument) {
        if (index < 0)
          throw std::out_of_range("index");
        if (count < 0)
	...
	ListBase<CTYPE>::iterator first = $self->begin();
	std::advance(first, index);
	ListBase<CTYPE>::iterator last = first;
	std::advance(last, count);
	ListBase<CTYPE>* pList = new ListBase<CTYPE>();
	pList->insert(pList->end(), first, last);
        return pList;
      }
      ...
      void InsertRange(int index, const ListBase< CTYPE >& values) throw (std::out_of_range) {
        if (index>=0 && index<(int)$self->size()+1) {
	  ListBase<CTYPE>::iterator it = $self->begin();
	  std::advance(it, index);
	...
	std::advance(last, count);
      ...
      static ListBase< CTYPE > *Repeat(CTYPE const& value, int count) throw (std::out_of_range) {
        if (count < 0)
          throw std::out_of_range("count");
	ListBase<CTYPE>* pList = new ListBase<CTYPE>();
	pList->assign(count, value);
        return pList;
      }
      void Reverse() {
	...
	std::advance(last, count);
      ...
      void SetRange(int index, const ListBase< CTYPE >& values) throw (std::out_of_range) {
        if (index < 0)
          throw std::out_of_range("index");
	...
	std::advance(it, index);
      ...
      int IndexOf(CTYPE const& value) {
        int index = -1;
        ListBase<CTYPE>::iterator it = std::find($self->begin(), $self->end(), value);
        if (it != $self->end())
      ...
      /*int LastIndexOf($typemap(cstype, CTYPE) const& value) {
        int index = -1;
        ListBase::reverse_iterator rit = std::find($self->rbegin(), $self->rend(), value);
        if (rit != $self->rend())
